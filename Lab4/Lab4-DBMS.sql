...
select count(customer.cus_gender),customer.cus_gender as count 
from customer
join `order`
...
SELECT COUNT(*) AS "TOTAL CUSTOMERS", cus_gender as gender from customer
where customer.cus_id in(SELECT distinct(cus_id) from `order` where ord_amount >= 3000)
group by cus_gender;
...
group by supplier.supp_id order by supplier.supp_id;

-- 6)	Find the least expensive product from each category and print the table with category id, name, product name and price of the product.

select category.cat_id, category.cat_name, min(supplier_pricing.supp_price) 
from product, supplier_pricing, category 
...
select cus_name as name,cus_gender as gender from customer
